...
| ADR-29 | Declare legs by kind, and impair an LTE leg at the radio | A scenario's LTE leg carries radio parameters and no rate, because in the simulator its capacity emerges. Its degradation is extra path loss in dB, widening the harness's impairment rule beyond the alternative leg so that the primary radio can be the one that fails. The finite world reads the same leg through a declared logical capacity and a loss-to-rate table; that table is an interpretation, nominal and uncalibrated until the simulator pilot measures what each loss does. A rate change on an LTE leg, or a radio loss on a point-to-point leg, is refused. SCADA is modelled as the request/response transaction v1 scope defines in both worlds, replacing the finite world's one-way simplification |
| ADR-30 | Degrade LTE by a cell-edge site in a busy cell under demand near its share | Measured in the simulator, path loss alone makes the leg binary for the v1 workload: SCADA was on time at every loss up to 49.5 dB and never arrived at 49.75 dB, and cell load alone left a site 461 kbit/s at the model's limit of 35 competing UEs. Proportional-fair scheduling serves a light site promptly, so degradation appears only when demand approaches the site's share: at 48 dB with 5 competing UEs and AMI every 100 ms, SCADA ran late, 1 of 56 on time and 48 late, rather than stopping. S1, S3 and S4 declare that condition; S2 remains an outage. `cell_load` is a disturbance on the LTE leg, bounded at 35 competitors; the finite world reads loss and load together through a measured rate table. AMI pacing becomes a lever that can matter, because it releases share SCADA needs |
